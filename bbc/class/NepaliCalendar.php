...
final class NepaliCalendar
{
	...
	public function eng_to_nep($yy, $mm, $dd)
	{
		if ($this->is_range_eng($yy, $mm, $dd) == false) {
			...
			$this->nep_date["year"] = $this->num_eng_to_nep($y);
			$this->nep_date["month"] = $this->num_eng_to_nep($m);
			$this->nep_date["date"] = $this->num_eng_to_nep($total_nDays);
			$this->nep_date["day"] = $this->get_nepali_day_of_week($day);
	...
	public function num_eng_to_nep($num)
	{
		$nums = array(
			"0" => "०",
			"1" => "१",
	...
	private function get_nepali_day_of_week($day)
	{
		switch ($day) {
			case 1:
				$day = "आइतवार";
	...
	private function get_nepali_short_day($day)
	{
		switch ($day) {
			case 1:
				$day = "आइत";
